...
class EnhancedParkingDetector:
    def __init__(self, video_path: Optional[str] = None, image_path: Optional[str] = None) -> None:
        ...
        # Initialize detector lazily so mapping can run even if ML extras are missing.
    ...
    def _normalize_slot(self, slot: Tuple[int, ...]) -> Tuple[int, int, int, int]:
        """Return slot as (x, y, w, h), supporting legacy (x, y) entries."""
        if isinstance(slot, (list, tuple)) and len(slot) >= 4:
            x, y, w, h = slot[:4]
            return int(x), int(y), max(1, int(w)), max(1, int(h))

        x, y = slot[:2]
        return int(x), int(y), int(self.width), int(self.height)

    def _screen_to_image(self, x: int, y: int) -> Optional[Tuple[int, int]]:
        """Map displayed window coordinates back to source image coordinates."""
        if self.current_image is None:
            return None

        # Use direct image-space mapping based on static border.
        src_x = int(x) - config.BORDER_SIZE
        src_y = int(y) - config.BORDER_SIZE
    ...
    def load_parking_positions(self) -> None:
        """Load previously saved parking positions from file (pickle or CSV)"""
        if not os.path.exists(config.POSITION_FILE):
            logger.warning(f"No saved positions found at '{config.POSITION_FILE}'.")
            self.posList = []
            return
        ...
        with open(config.POSITION_FILE, "rb") as f:
            try:
                ...
                logger.info(f"Loaded {len(self.posList)} parking positions (pickle)")
            except Exception:
                # Fallback: file is plain text with x,y per line (CSV)
                f.seek(0)
                try:
                    ...
                    for line in lines:
                        ...
                        if line and "," in line:
                            coords = line.split(",")
                            if len(coords) == 2:
                                pos.append((int(coords[0]), int(coords[1]), self.width, self.height))
                            elif len(coords) >= 4:
                                pos.append(
                                    (
                                        int(coords[0]),
                    ...
                    self.posList = pos
                    logger.info(f"Loaded {len(self.posList)} parking positions (CSV fallback)")
                except Exception as e:
                    logger.error(f"Error loading positions as CSV: {e}")
                    self.posList = []

    def save_parking_positions(self) -> None:
        """Save current parking positions to file as CSV"""
        try:
            self.history.append(self.posList.copy())
            with open(config.POSITION_FILE, "w") as f:
                for pos in self.posList:
                    x, y, w, h = self._normalize_slot(pos)
                    f.write(f"{x},{y},{w},{h}\n")
            logger.info(f"Saved {len(self.posList)} parking positions to {config.POSITION_FILE} (CSV)")
        except Exception as e:
    ...
    def mouse_callback(self, event: int, x: int, y: int, flags: int, param: Any) -> None:
        ...
        mapped = self._screen_to_image(x, y)

        # Ignore clicks outside the actual image area.
        if mapped is None and event != cv2.EVENT_LBUTTONUP:
            return
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if self.drawing and mapped is not None:
                x, y = mapped
                self.end_point = (x, y)
        elif event == cv2.EVENT_LBUTTONUP:
            self.drawing = False
            ...
            if self.start_point and self.end_point:
                ...
                # Small drag behaves like a click with default slot size.
                if dist_x < 10 and dist_y < 10:
                    pos = (x1, y1, self.width, self.height)
                    if pos not in self.posList:
                        self.posList.append(pos)
                else:
                    # Keep exact drag rectangle: no grid snapping/tiling.
                    pos = (x1, y1, max(1, dist_x), max(1, dist_y))
                    if pos not in self.posList:
                        self.posList.append(pos)
            ...
            for pos in self.posList[:]:
                x1, y1, w, h = self._normalize_slot(pos)
    ...
    def check_parking_space(
        ...
        for pos in self.posList:
            ...
            # Scale threshold by selected slot area for custom-sized boxes.
            area_scale = (w * h) / float(self.width * self.height)
            slot_threshold = int(max(1, config.OCCUPANCY_THRESHOLD * area_scale))
    ...
    def generate_csv_report(
        ...
        try:
            if pd is None:
                logger.warning("pandas is not installed. Skipping CSV report generation.")
                return
    ...
    def process_image(self) -> None:
        ...
        # Keep a fixed-size display to avoid mouse coordinate drift from runtime scaling.
            ...
            while True:
                ...
                if key == ord("q"):
                    ...
                    if CarDetector is None:
                        logger.warning(
                            "ML dependencies are missing. Install Car-Parking-Detection requirements to use detection."
                        )
                        print(
                            "ML dependencies are missing. Install requirements to use detection (D)."
                        )
                        continue
                    ...
                    legacy_positions = [
                        (self._normalize_slot(pos)[0], self._normalize_slot(pos)[1])
                        for pos in self.posList
                    ]
                    detections, space_status = self.car_detector.process_detections(
                        results, legacy_positions, self.current_image
                    )

                    print("📊 Generating visual report...")
                    # Generate comprehensive report
                    report_path, text_report_path = self.car_detector.generate_report(
                        self.current_image, legacy_positions, detections, space_status
                    )
